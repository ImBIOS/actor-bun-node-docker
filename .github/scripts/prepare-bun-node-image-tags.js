module.exports = () => {
    ...
    if (RELEASE_TAG === "latest") {
        if (FRAMEWORK_VERSION) {
            ...
            // apify/actor-node-x:1-20-4.2.0
            tags.push(
                `${IMAGE_NAME}:${LATEST_BUN}-${CURRENT_NODE}-${FRAMEWORK_VERSION}`
            );
        }

        // apify/actor-node-x:20
        // apify/actor-node-x:1-20
        // we want this only when the browser image is also latest
        if (IS_LATEST_BROWSER_IMAGE === "true") {
            tags.push(`${IMAGE_NAME}:${CURRENT_NODE}`);
            tags.push(`${IMAGE_NAME}:${LATEST_BUN}-${CURRENT_NODE}`);
        }
    } else {
        // beta and other tags
        if (FRAMEWORK_VERSION) {
